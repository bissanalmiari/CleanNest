...
export default function PayBookingPage({ params }: { params: Promise<{ bookingId: string }> }) {
  ...
  const handlePayWithStripe = async () => {
    ...
    try {
      const res = await fetch(`/api/customer/payments/booking/${bookingId}/checkout`, {
        method: "POST",
      });
  ...
  return (
    <div className="min-h-screen bg-surface-soft p-6">
      <div className="mx-auto max-w-md space-y-5">
        ...
        {wasCanceled && (
          <Alert variant="error">
            Checkout was canceled before payment completed. You can try again below.
          </Alert>
        )}

        <div className="space-y-4 rounded-card bg-surface p-5 shadow-card">
          <p className="text-sm text-navy/70">
            You&apos;ll be securely redirected to Stripe to enter your card details. CleanNest never
            sees or stores your card number.
          </p>

          <div className="flex items-center gap-2 rounded-md bg-navy/5 px-3 py-2 text-xs text-navy/60">
            <Lock size={14} />
            Payments are processed by Stripe, a PCI-compliant payment provider.
          </div>
