...
import { Search, Users, Mail, Phone, Calendar, ChevronLeft, ChevronRight } from "lucide-react";
...
const ROW_ACCENT: Record<string, string> = {
  ...
  inactive: "border-l-navy/15",
  suspended: "border-l-status-cancelled",
};

const AVATAR_RING: Record<string, string> = {
  admin: "ring-primary/30",
  cleaner: "ring-status-inProgress/30",
  customer: "ring-status-confirmed/30",
};
  ...
  const fetchUsers = useCallback(
    async (currentFilters: FiltersState, currentPage: number) => {
      ...
      try {
        ...
        const json: ApiEnvelope<UserListData> = await res.json();
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-sm text-navy/55">
              Search, filter, and manage customer, cleaner, and admin accounts.
            </p>
          </div>
          {data && !loading && (
            <span className="ml-auto flex items-center gap-1.5 rounded-full border border-navy/10 bg-surface px-3.5 py-1.5 text-sm font-semibold text-navy shadow-sm">
                  ...
                  data.users.map((user) => (
                    <tr
                      ...
                      className={`group cursor-pointer border-b border-l-[3px] border-navy/[0.05] last:border-b-0 transition-colors hover:bg-surface-soft/60 ${
                        ROW_ACCENT[user.status] ?? "border-l-transparent"
                      }`}
                    >
                      <td className="py-3.5 pl-6 pr-3">
                        <span className="flex items-center gap-3 font-semibold text-navy">
                          <span
                            className={`flex h-8 w-8 items-center justify-center rounded-full bg-primary-light text-[11px] font-bold text-primary ring-2 ${
                              AVATAR_RING[user.role] ?? "ring-navy/10"
                            }`}
                          >
                            {initials(user.name)}
                          </span>
                      ...
                      <td className="px-3 py-3.5 text-navy/60">{user.email}</td>
                      ...
                      <td className="py-3.5 pl-3 pr-6 text-navy/45">
          ...
          {data && totalPages > 1 && (
            <div className="flex items-center justify-between border-t border-navy/[0.06] bg-surface-soft/40 px-6 py-3.5 text-sm">
              <span className="text-navy/50">
                ...
                of {totalPages} · {data.total} users
              </span>
