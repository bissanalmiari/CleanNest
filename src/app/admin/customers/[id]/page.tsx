...
export default function AdminCustomerDetailPage() {
  ...
  async function updateCustomer(values: CustomerFormValues) {
    if (!data) return;
    ...
    const json: ApiEnvelope<CustomerDetail> = await response.json();

    if (!response.ok || !json.success) {
      throw new Error(json.error ?? "Customer could not be updated.");
    }
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#f3f7fc] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
      ...
      <div className="relative mx-auto max-w-[1250px] space-y-6">
        <Link
          href="/admin/customers"
          className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-xs font-extrabold text-navy shadow-sm transition hover:border-primary/30 hover:text-primary"
        >
          <ArrowLeft className="h-4 w-4" />
          Back to customers
        </Link>
        ...
        <motion.section
          ...
          className="relative overflow-hidden rounded-[2.25rem] bg-[linear-gradient(125deg,#071d38_0%,#0b315d_52%,#1675cf_100%)] p-6 text-white shadow-[0_30px_90px_rgba(11,37,69,0.22)] sm:p-8"
        >
          <div className="absolute -right-20 -top-32 h-80 w-80 rounded-full border border-cyan-200/20 bg-cyan-300/10" />
          <div className="relative flex flex-col gap-7 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
              <span className="relative flex h-24 w-24 shrink-0 items-center justify-center rounded-[1.75rem] bg-cyan-300 font-heading text-2xl font-black text-navy shadow-[0_20px_45px_rgba(34,211,238,0.2)]">
            ...
            <div className="flex flex-wrap gap-3">
              <button
                ...
                className="inline-flex min-h-12 items-center gap-2 rounded-2xl bg-white px-5 text-xs font-extrabold text-navy transition hover:bg-cyan-50"
              >
          ...
          <aside className="space-y-5">
            <div className="rounded-[2rem] border border-slate-200/80 bg-white p-6 shadow-[0_20px_60px_rgba(11,37,69,0.08)]">
              ...
              <p className="mt-1 text-sm font-medium text-slate-500">
                Total bookings connected to this account
              </p>
              <Link
                href="/admin/bookings"
                className="group mt-6 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-navy text-xs font-extrabold text-white transition hover:bg-primary"
              >
