...
import { bookingIdParamsSchema, cancelBookingSchema } from "@/validators/bookingValidator";
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (currentUser.role !== "customer") {
      throw new AppError("Only customers can cancel bookings from this endpoint.", 403);
    }
     ...
     */
    const bodyValidation = cancelBookingSchema.safeParse(requestBody);
