...
import { bookingIdParamsSchema, rescheduleBookingSchema } from "@/validators/bookingValidator";
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (currentUser.role !== "customer") {
      throw new AppError("Only customers can reschedule bookings from this endpoint.", 403);
    }
     ...
     */
    const bodyValidation = rescheduleBookingSchema.safeParse(requestBody);
