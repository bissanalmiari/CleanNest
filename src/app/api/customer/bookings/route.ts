...
export async function GET() {
  try {
    ...
    if (currentUser.role !== "customer") {
      throw new AppError("Only customers can view bookings from this endpoint.", 403);
    }
    ...
    const bookingDocuments = await BookingModel.find({
    ...
    const serviceIds = [
      ...new Set(bookingDocuments.map((booking) => booking.serviceId.toString())),
    ].map((serviceId) => new Types.ObjectId(serviceId));

    const addressIds = [
      ...new Set(bookingDocuments.map((booking) => booking.addressId.toString())),
    ].map((addressId) => new Types.ObjectId(addressId));

    const bookingIds = bookingDocuments.map((booking) => booking._id);

    const [serviceDocuments, addressDocuments, reviewDocuments] = await Promise.all([
      ServiceModel.find({
        _id: {
          $in: serviceIds,
        },
      ...
      AddressModel.find({
        ...
        .select("_id label city area street")
        .lean()
        .exec(),
    ...
    const servicesById = new Map<string, ServiceSummary>(
    ...
    const addressesById = new Map<string, AddressSummary>(
      addressDocuments.map((address) => [
    ...
    const bookings = bookingDocuments.map((booking) => {
      const service = servicesById.get(booking.serviceId.toString());

      const address = addressesById.get(booking.addressId.toString());
      ...
      return {
        ...
        bookingDate: formatDateInBeirut(new Date(booking.bookingDate)),
        ...
        serviceAreaFee: booking.serviceAreaFee,
        ...
        addressLabel: address
          ? [address.label, address.area, address.city].filter(Boolean).join(" · ")
          : "Address unavailable",
