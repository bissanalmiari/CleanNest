...
export default function DashboardSidebar({
  ...
  const roleTitle = role === "admin" ? "Administrator" : "Customer";

  const roleDescription = role === "admin" ? "Management workspace" : "Cleaning account";
  ...
  function renderSidebarContent(isCollapsed: boolean, isMobile = false) {
    return (
      <div className="relative flex h-full w-full min-w-0 flex-col overflow-hidden bg-gradient-to-b from-[#071a33] via-[#0b2545] to-[#0d315c] text-white">
          ...
          >
            ...
            <AnimatePresence initial={false}>
              {!isCollapsed && (
                ...
                >
                  <p className="whitespace-nowrap font-heading text-xl font-extrabold">CleanNest</p>
            ...
            <AnimatePresence initial={false}>
              {!isCollapsed && (
                <motion.div
                  ...
                  className="min-w-0"
                >
                  <p className="truncate text-sm font-bold">{roleTitle}</p>

                  <p className="mt-0.5 truncate text-[11px] text-blue-100/55">{roleDescription}</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
        ...
        {/* Navigation */}
        <nav className="relative z-10 mt-5 min-h-0 min-w-0 flex-1 overflow-y-auto overflow-x-hidden px-3 pb-5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {!isCollapsed && (
              ...
              return (
                ...
                >
                  ...
                  >
                    {active && (
                      <motion.span
                        layoutId={`dashboard-active-navigation-${isMobile ? "mobile" : "desktop"}`}
                    ...
                    {!isCollapsed && item.badge !== undefined && (
                      <span
                        className={`relative z-10 flex min-w-6 shrink-0 items-center justify-center rounded-full px-1.5 py-0.5 text-[10px] font-extrabold ${
                          active ? "bg-white/20 text-white" : "bg-primary/20 text-cyan-200"
                        }`}
                      >
                        {item.badge}
                      </span>
        ...
        <div className="relative z-10 min-w-0 shrink-0 overflow-hidden border-t border-white/10 p-3">
          ...
          >
                ...
                {!isCollapsed && (
                  ...
                  >
                    ...
                    <p className="mt-0.5 truncate text-[11px] text-blue-100/50">
                      {currentUser.email ?? roleDescription}
                    </p>
                  </motion.div>
                )}
              </AnimatePresence>
            ...
            {onLogout && (
              ...
              >
                <LogOut className={`h-4 w-4 shrink-0 ${logoutLoading ? "animate-pulse" : ""}`} />

                {!isCollapsed && (
                  <span className="truncate">{logoutLoading ? "Logging out..." : "Log out"}</span>
                )}
              </motion.button>
            )}
          </div>
