...
function createDefaultValues(user: PublicUser): UpdateProfileValues {
  return {
    name: user.name,
...
export function EditProfileForm({ user, onSaved }: EditProfileFormProps) {
  const { updateProfile, loading, error, setError } = useProfile();
  ...
  const bioValue = typeof watchedBio === "string" ? watchedBio : "";
  ...
  async function onSubmit(values: UpdateProfileValues) {
    ...
    reset(createDefaultValues(updatedUser));
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-7" noValidate>
      ...
      {error && (
        ...
        >
          ...
          <div>
            <p className="font-extrabold">Profile could not be updated</p>
      ...
      {savedSuccessfully && (
        ...
        >
          ...
          <div>
            <p className="font-extrabold">Profile updated</p>

            <p className="mt-1 text-sm font-semibold leading-6">
              Your personal information was saved successfully.
            </p>
          </div>
        </div>
      )}
      ...
      <section className="rounded-[1.7rem] border border-primary/10 bg-[#f8fbfe] p-5 sm:p-6">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-2 text-sm font-medium leading-6 text-slate-500">
              Keep your contact information accurate for booking updates and service communication.
            </p>
          </div>
        </div>
        ...
        <div className="mt-7 grid gap-5 lg:grid-cols-2">
          ...
          <div className="lg:col-span-2">
            <FormFieldShell icon={Mail} label="Email address" htmlFor="profile-email">
      ...
      <section className="rounded-[1.7rem] border border-primary/10 bg-[#f8fbfe] p-5 sm:p-6">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <h3 className="mt-2 font-heading text-xl font-black text-navy">
              A profile shaped around you
            </h3>

            <p className="mt-2 text-sm font-medium leading-6 text-slate-500">
              These optional details help personalize your CleanNest account experience.
            </p>
          </div>
        </div>
          ...
          <div className="lg:col-span-2">
            ...
            >
              <select
                ...
                className={fieldClass(Boolean(errors.preferredLanguage))}
              >
        ...
        <div className="mt-7">
          ...
          >
            <textarea
              ...
              {...register("bio")}
              className={`${fieldClass(Boolean(errors.bio))} resize-none py-4 leading-7`}
            />

            <div className="mt-2 flex items-center justify-between gap-4">
              <p className="text-xs font-medium text-slate-400">Add up to 300 characters.</p>

              <p
                className={`text-xs font-extrabold ${
                  bioValue.length > 270 ? "text-amber-500" : "text-slate-400"
                }`}
              >
                {bioValue.length}/300
              </p>
      ...
      <section className="flex flex-col gap-4 rounded-[1.7rem] border border-primary/10 bg-white p-5 shadow-[0_14px_35px_rgba(11,37,69,0.06)] sm:flex-row sm:items-center sm:justify-between sm:p-6">
        <div>
          <p className="font-heading text-lg font-black text-navy">Save your profile</p>

          <p className="mt-1 text-sm font-medium leading-6 text-slate-500">
            Changes are applied to your CleanNest account immediately.
          </p>
        </div>
