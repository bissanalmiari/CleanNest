...
function normalizeAddress(
  ...
  return {
    ...
    serviceAreaId: readString(
      record.serviceAreaId,
    ),

    serviceAreaLabel: readString(
      record.serviceAreaLabel,
    ),
    ...
    maximumConcurrentBookings:
      Math.max(
        0,
        readNumber(
          record.maximumConcurrentBookings,
        ),
      ),
  };
}
...
function normalizeAddressArray(
  ...
  return value
    ...
    .filter(
      (
        address,
      ): address is HomeBaseAddress =>
        address !== null,
    );
}

function extractAddresses(
  payload: AddressApiResponse,
): HomeBaseAddress[] {
  ...
  if (
    nestedAddresses.length > 0
  ) {
    return nestedAddresses;
  }

  return normalizeAddressArray(
    payload.addresses,
  );
}
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-2 text-base font-semibold leading-7 text-emerald-700">
                ...
                {
                  selectedAddress.serviceAreaLabel
                }{" "}
                ·{" "}
                {formatCurrency(
                  selectedAddress.serviceFee,
                )}
              </p>
            </div>
          </div>
