...
const CLEAN_NEST_TIME_ZONE = "Asia/Beirut";

const OPENING_TIME_MINUTES = 8 * 60;

const CLOSING_TIME_MINUTES = 18 * 60;
...
type SlotStatus = "checking" | "available" | "unavailable";
...
interface TimeRouteStepProps {
  ...
  onChange: (selection: TimeRouteSelection) => void;

  onTrustedQuote?: (snapshot: TimeRoutePricingSnapshot) => void;
}
...
function getBeirutCurrentMinutes(): number {
  const { hour, minute } = getBeirutDateParts();

  return hour * 60 + minute;
}

function addDaysToDateString(dateString: string, numberOfDays: number): string {
...
function createDateOption(value: string): DateOption {
  const [year, month, day] = value.split("-").map(Number);

  const date = new Date(Date.UTC(year ?? 2000, (month ?? 1) - 1, day ?? 1, 12));
  const closure = getFullDayScheduleBlock(value);

  return {
    value,
    isClosed: Boolean(closure),
    closureReason: closure?.reason ?? null,
    isFriday: isFridaySchedule(value),
...
function isPastSlot(bookingDate: string, startTime: string): boolean {
  ...
  return timeToMinutes(startTime) <= getBeirutCurrentMinutes();
}

function buildTimeSlots(durationMinutes: number): AvailabilitySlot[] {
  ...
  for (
    let startMinutes = OPENING_TIME_MINUTES;
    startMinutes + durationMinutes <= CLOSING_TIME_MINUTES;
...
function extractQuote(payload: ApiResponse): Record<string, unknown> | null {
  const dataRecord = asRecord(payload.data);

  return asRecord(dataRecord?.quote ?? payload.quote);
}

function extractAvailability(payload: ApiResponse): Record<string, unknown> | null {
  const dataRecord = asRecord(payload.data);

  return asRecord(dataRecord?.availability ?? payload.availability);
}
...
export default function TimeRouteStep({
  ...
  const onTrustedQuoteRef = useRef(onTrustedQuote);

  const selectedDateRef = useRef(bookingDate);

  const selectedStartTimeRef = useRef(startTime);

  const selectedEndTimeRef = useRef(endTime);

  const today = useMemo(() => getBeirutToday(), []);
  ...
  const dateOptions = useMemo(() => {
    return Array.from(
      ...
      (_, index) => createDateOption(addDaysToDateString(today, index))
    );
  }, [today]);
  ...
  const pricingPayload = useMemo(
    ...
    [addOns, bathrooms, bedrooms, propertySize, propertyType, serviceId]
  );

  const [selectedDate, setSelectedDate] = useState(bookingDate);

  const [trustedDurationMinutes, setTrustedDurationMinutes] = useState<number | null>(null);
  ...
  useEffect(() => {
    onTrustedQuoteRef.current = onTrustedQuote;
  }, [onTrustedQuote]);

  useEffect(() => {
    selectedDateRef.current = bookingDate;
  ...
  useEffect(() => {
    if (!bookingDate || !getFullDayScheduleBlock(bookingDate)) {
      return;
    }
    ...
    setSelectedDate("");
    setSlots([]);
    setAvailabilityError("CleanNest is closed every Sunday. Please choose another day.");
    onChangeRef.current({
      bookingDate: "",
      startTime: "",
      endTime: "",
  ...
  useEffect(() => {
    ...
    const requestVersion = quoteVersionRef.current;

    const controller = new AbortController();

    if (!serviceId) {
      setTrustedDurationMinutes(null);

      setQuoteError("Select a cleaning plan before choosing a time.");
    ...
    async function loadTrustedQuote() {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            extractErrorMessage(payload, "The trusted cleaning duration could not be calculated.")
          );
        }
        ...
        const estimatedDurationMinutes = readNumber(quote.estimatedDurationMinutes);

        if (estimatedDurationMinutes <= 0) {
          throw new Error("The trusted cleaning duration is invalid.");
        }

        if (requestVersion !== quoteVersionRef.current) {
          return;
        }

        setTrustedDurationMinutes(estimatedDurationMinutes);

        onTrustedQuoteRef.current?.({
          baseAmount: readNumber(quote.baseAmount),

          addOnsAmount: readNumber(quote.addOnsAmount),

          discountAmount: readNumber(quote.discountAmount),

          totalAmount: readNumber(quote.totalAmount),

          estimatedDurationMinutes,
        });

        const existingStartTime = selectedStartTimeRef.current;

        const existingEndTime = selectedEndTimeRef.current;

        if (existingStartTime && existingEndTime) {
          const trustedEndTime = calculateEndTime(existingStartTime, estimatedDurationMinutes);

          if (trustedEndTime !== existingEndTime) {
            onChangeRef.current({
              bookingDate: selectedDateRef.current,
        ...
        if (requestVersion === quoteVersionRef.current && !controller.signal.aborted) {
          setIsLoadingQuote(false);
        }
      }
    }
  ...
  useEffect(() => {
    ...
    const requestVersion = availabilityVersionRef.current;

    const controller = new AbortController();

    if (!selectedDate || !serviceId || !serviceAreaId || !trustedDurationMinutes) {
      setSlots([]);
    ...
    const baseSlots = buildTimeSlots(trustedDurationMinutes);

    const preparedSlots = baseSlots.map((slot) => {
      ...
      const recurringBlock = getRecurringScheduleBlock({
    ...
    async function checkSlot(slot: AvailabilitySlot): Promise<AvailabilitySlot> {
      if (slot.status === "unavailable") {
        return slot;
      }

      try {
        const response = await fetch("/api/customer/bookings/availability", {
          method: "POST",
        ...
        if (responseText.trim()) {
          try {
            ...
            return {
              ...
              message: "The availability server returned an invalid response.",
        ...
        if (!response.ok) {
          return {
            ...
            message: extractErrorMessage(payload, "This time could not be checked."),
        ...
        if (!availability) {
          return {
            ...
            message: "Availability information was missing.",

            requestFailed: true,
          };
        }

        const capacity = asRecord(availability.capacity);
        ...
        return {
          ...
          remainingCapacity: capacity ? readNumber(capacity.remainingCapacity) : null,

          blockedPeriod: availability.blockedPeriod === true,
        ...
        return {
          ...
          message: "Unable to reach the availability server.",
    ...
    async function checkAllSlots() {
      const checkedSlots = await Promise.all(preparedSlots.map(checkSlot));

      if (controller.signal.aborted || requestVersion !== availabilityVersionRef.current) {
        return;
      }
      ...
      const checkedRequests = checkedSlots.filter(
        (slot) => !isPastSlot(selectedDate, slot.startTime)
      );

      const everyRequestFailed =
        checkedRequests.length > 0 && checkedRequests.every((slot) => slot.requestFailed);

      if (everyRequestFailed) {
        setAvailabilityError(checkedRequests[0]?.message ?? "Availability could not be checked.");
      }

      const selectedStartTime = selectedStartTimeRef.current;

      const selectedEndTime = selectedEndTimeRef.current;

      if (selectedStartTime && selectedEndTime) {
        const stillAvailable = checkedSlots.some(
          (slot) =>
  ...
  }, [refreshNumber, selectedDate, serviceAreaId, serviceId, trustedDurationMinutes]);

  const selectedDateOption = useMemo(
    () => createDateOption(selectedDate || today),
    [selectedDate, today]
  );
  ...
  const selectedSlot = useMemo(
    () =>
      slots.find(
        (slot) =>
          slot.startTime === startTime && slot.endTime === endTime && slot.status === "available"
      ) ?? null,
  ...
  function handleDateSelection(nextDate: string) {
    ...
    const fullDayBlock = getFullDayScheduleBlock(nextDate);

    if (fullDayBlock) {
      setAvailabilityError(fullDayBlock.reason);
      return;
    }
  ...
  function handleSlotSelection(slot: AvailabilitySlot) {
    ...
    selectedEndTimeRef.current = slot.endTime;
    ...
    return (
      <div className="mt-8 rounded-[1.8rem] border border-amber-200 bg-amber-50 p-7 text-center">
        ...
        <p className="mx-auto mt-3 max-w-xl text-base font-medium leading-7 text-amber-700">
          Select a cleaning plan and a serviceable address before choosing a time.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="mt-8 space-y-8">
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-3 max-w-2xl text-base font-medium leading-7 text-slate-500">
              The duration now includes your property details and every selected extra before
        ...
        <div className="rounded-2xl border border-primary/10 bg-white px-5 py-4">
          ...
          {isLoadingQuote ? (
            ...
            <p className="mt-2 text-lg font-extrabold text-navy">
              {trustedDurationMinutes ? formatDuration(trustedDurationMinutes) : "Unavailable"}
            </p>
          )}
        </div>
      </section>
      ...
      {!quoteError && trustedDurationMinutes && (
        <>
          <section>
            ...
            <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4 xl:grid-cols-7">
              {dateOptions.map((option, index) => {
                ...
                const isClosed = option.isClosed;
                ...
                return (
                  <motion.button
                    ...
                    title={
                      ...
                      (option.isFriday ? "Friday prayer break: 12:00 PM–2:00 PM" : undefined)
                    }
                    onClick={() => {
                      handleDateSelection(option.value);
                    }}
                    initial={
                      prefersReducedMotion || isClosed
                        ? undefined
                        : {
                            opacity: 0,
                            y: 14,
                  ...
                  >
                    ...
                    <p className="mt-3 font-heading text-3xl font-black">{option.day}</p>
          ...
          <section className="rounded-[1.7rem] border border-primary/10 bg-white p-5 sm:p-6">
            <div className="flex flex-col gap-4 border-b border-primary/10 pb-5 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <h3 className="mt-2 font-heading text-2xl font-black text-navy">
                  {selectedDate ? selectedDateOption.fullLabel : "Select a date first"}
                </h3>
              </div>
              ...
              {selectedDate && (
                <div className="flex items-center gap-3">
                  ...
                  >
                    <RefreshCw
                      className={`h-4 w-4 ${isCheckingAvailability ? "animate-spin" : ""}`}
                    />
                  </button>
                </div>
              )}
            ...
            {!selectedDate ? (
              <div className="py-14 text-center">
                ...
                <p className="mt-4 text-base font-bold text-slate-500">
                  Choose a date to view available arrival times.
                </p>
              </div>
            ) : slots.length === 0 && isCheckingAvailability ? (
              <div className="py-14 text-center">
                <LoaderCircle className="mx-auto h-10 w-10 animate-spin text-primary" />

                <p className="mt-4 text-base font-bold text-slate-500">
                  Checking CleanNest capacity…
                </p>
              </div>
            ) : slots.length === 0 ? (
              <div className="py-14 text-center">
                ...
                <p className="mt-4 text-base font-bold text-slate-600">
                  The trusted cleaning duration cannot finish within the current booking hours.
                </p>
              </div>
            ) : (
                ...
                {slots.map((slot) => {
                  ...
                  return (
                    <motion.button
                      ...
                      whileHover={
                        prefersReducedMotion || !isAvailable
                          ? undefined
                          : {
                              y: -3,
                            }
                    ...
                    >
                      ...
                      >
                        Ends {formatClockTime(slot.endTime)}
                      </p>
            ...
            {availabilityError && (
              <div className="mt-5 flex items-start gap-3 rounded-2xl border border-red-200 bg-red-50 p-4">
                ...
                <p className="text-sm font-semibold leading-6 text-red-600">{availabilityError}</p>
              </div>
            )}
          </section>
          ...
          {selectedSlot && (
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div>
                  <p className="text-lg font-extrabold text-emerald-900">Time route reserved</p>

                  <p className="mt-2 text-base font-semibold leading-7 text-emerald-700">
                    {selectedDateOption.fullLabel} · {formatClockTime(selectedSlot.startTime)} to{" "}
