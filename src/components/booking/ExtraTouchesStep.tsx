...
function normalizeAddOnArray(
  ...
  return value
    ...
    .filter(
      (
        addOn,
      ): addOn is ExtraTouchOption =>
        addOn !== null,
    );
}

function extractAddOns(
  payload: AddOnsApiResponse,
): ExtraTouchOption[] {
  const nestedAddOns =
    normalizeAddOnArray(
      payload.data?.addOns,
...
export default function ExtraTouchesStep({
  ...
  const loadAddOns =
    useCallback(async () => {
      if (!serviceId) {
        ...
        setErrorMessage(
          "Choose a cleaning plan before loading extra touches.",
        );
      ...
      try {
        const response = await fetch(
          `/api/customer/services/${encodeURIComponent(
            serviceId,
          )}/add-ons`,
          {
            method: "GET",
        ...
        if (!response.ok) {
          throw new Error(
            payload.error ??
              payload.message ??
              "Unable to load extra touches.",
          );
        }

        setAddOns(
          extractAddOns(payload),
        );
      } catch (error) {
        setAddOns([]);
  ...
  function toggleAddOn(
    addOn: ExtraTouchOption,
  ) {
    const existingSelection =
      getSelection(addOn.id);
      ...
      {addOns.length === 0 ? (
        <section className="rounded-[1.7rem] border border-amber-200 bg-amber-50 p-7 text-center">
          ...
          <p className="mx-auto mt-3 max-w-xl text-base font-medium leading-7 text-amber-700">
            This cleaning plan currently
            has no linked add-ons. You
            can continue directly to the
            address stage.
          </p>
        </section>
      ) : (
