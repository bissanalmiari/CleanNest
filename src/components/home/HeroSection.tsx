...
const cleaningRooms: CleaningRoom[] = [
  {
    ...
    icon: Home,
    className: "col-start-1 row-start-1 rounded-[2rem_1rem_1rem_1rem]",
    accentClassName: "from-blue-500/20 via-primary/10 to-transparent",
  },
  {
    name: "Kitchen",
    detail: "Counters and cooking area",
    icon: CookingPot,
    className: "col-start-2 row-start-1 rounded-[1rem_2rem_1rem_1rem]",
  ...
  },
  {
    ...
    icon: BedDouble,
    className: "col-start-1 row-start-2 rounded-[1rem_1rem_1rem_2rem]",
    accentClassName: "from-indigo-500/20 via-primary/10 to-transparent",
  },
  {
    name: "Bathroom",
    detail: "Sanitized and polished",
    icon: Bath,
    className: "col-start-2 row-start-2 rounded-[1rem_1rem_2rem_1rem]",
...
const cleaningStages: CleaningStage[] = [
  {
    ...
    description: "The cleaning route begins with the main shared space.",
  },
  {
    title: "Detailing the kitchen",
    shortTitle: "Kitchen",
    description: "High-use surfaces receive focused cleaning and care.",
  },
  {
    title: "Resetting the bedroom",
    shortTitle: "Bedroom",
    description: "The private space is refreshed and neatly organized.",
  },
  {
    title: "Finishing the bathroom",
    shortTitle: "Bathroom",
    description: "The final zone is sanitized before the quality check.",
  },
];
...
function roomStateClasses(roomIndex: number, activeStage: number) {
  const isActive = roomIndex === activeStage;

  const isComplete = roomIndex < activeStage;

  if (isActive) {
    return {
      room: "border-primary/50 bg-white shadow-[0_22px_55px_rgba(30,111,217,0.20)] ring-4 ring-primary/10",
      icon: "bg-primary text-white",
      status: "border-primary/15 bg-primary-light text-primary",
    };
  }

  if (isComplete) {
    return {
      room: "border-emerald-200 bg-emerald-50/80 shadow-[0_16px_40px_rgba(16,185,129,0.10)]",
      icon: "bg-emerald-500 text-white",
      status: "border-emerald-200 bg-white text-emerald-600",
    };
  }

  return {
    room: "border-slate-200/80 bg-white/70 shadow-[0_14px_35px_rgba(11,37,69,0.07)]",
    icon: "bg-slate-100 text-slate-400",
    status: "border-slate-200 bg-white/80 text-slate-400",
  };
}
...
export default function HeroSection() {
  ...
  const plannerRotateX = useTransform(smoothPointerY, [-0.5, 0.5], [5, -5]);

  const plannerRotateY = useTransform(smoothPointerX, [-0.5, 0.5], [-6, 6]);

  const leftGlowX = useTransform(smoothPointerX, [-0.5, 0.5], [-35, 35]);

  const leftGlowY = useTransform(smoothPointerY, [-0.5, 0.5], [-25, 25]);

  const routeProgress = ((activeStage + 1) / cleaningStages.length) * 100;

  const currentCleaningStage = cleaningStages[activeStage] ?? cleaningStages[0]!;

  const currentMarkerPosition = cleanerMarkerPositions[activeStage] ?? cleanerMarkerPositions[0]!;

  useEffect(() => {
    if (prefersReducedMotion) {
      setActiveStage(cleaningStages.length - 1);

      return;
    }

    const interval = window.setInterval(() => {
      setActiveStage((currentStage) => (currentStage + 1) % cleaningStages.length);
    }, 2600);
  ...
  return (
    ...
    >
      ...
      <div aria-hidden="true" className="pointer-events-none absolute inset-0">
        {backgroundCoordinates.map((coordinate, index) => (
          <motion.span
      ...
      <div className="absolute left-5 top-1/2 z-10 hidden -translate-y-1/2 xl:block">
        <div className="flex items-center gap-4 [writing-mode:vertical-rl]">
          ...
          <span className="font-heading text-xs font-bold text-navy/35">001</span>
        </div>
      </div>
      ...
      <div className="relative mx-auto grid min-h-[calc(100vh-72px)] max-w-[1450px] items-center gap-12 px-5 pb-14 pt-24 sm:px-8 lg:grid-cols-[0.88fr_1.12fr] lg:px-10 lg:pb-16 lg:pt-28 xl:gap-16">
        ...
        <div className="relative z-10 max-w-2xl">
          ...
          >
            Select a service and preferred time. CleanNest organizes the rest into one clear
            journey—from booking to a freshly cleaned space.
          </motion.p>
          ...
          >
            {["Choose", "Schedule", "Relax", "Return clean"].map((step, index) => (
              <div key={step} className="flex items-center gap-2">
                ...
                {index < 3 && <ArrowRight className="h-3.5 w-3.5 text-primary/35" />}
              </div>
            ))}
          </motion.div>
          ...
          >
            ...
            >
              ...
              >
                ...
                <span className="relative">Build My Cleaning</span>
          ...
          >
            {trustItems.map(({ icon: Icon, text }, index) => (
              ...
              >
                ...
                <span className="text-xs font-semibold leading-5 text-slate-500">{text}</span>
              </motion.div>
            ))}
          </motion.div>
        </div>
        ...
        >
          ...
          <div className="mb-3 flex items-center justify-between px-2">
            ...
            <p className="font-mono text-[10px] font-bold text-slate-400">CN / HOME / 04</p>
          </div>
          ...
          >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <AnimatePresence mode="wait">
                    <motion.p
                      key={currentCleaningStage.title}
                      initial={{
                        opacity: 0,
                        y: 8,
                      }}
            ...
            <div className="relative mt-3 aspect-[620/430] overflow-hidden rounded-[1.8rem] border border-primary/10 bg-[#eef5fc] p-3 sm:p-4">
              ...
              >
                ...
                <motion.path
                  ...
                  animate={{
                    pathLength: prefersReducedMotion ? 1 : [0.05, 0.35, 0.62, 1],
                    opacity: 1,
                  }}
                  transition={{
                    duration: 8,
                    repeat: prefersReducedMotion ? 0 : 0,
                    ease: "easeInOut",
                  }}
                />

                <defs>
                  <linearGradient id="cleanRouteGradient" x1="0" y1="0" x2="620" y2="430">
              ...
              <div className="relative z-10 grid h-full grid-cols-[1.12fr_0.88fr] grid-rows-2 gap-2.5 sm:gap-3">
                {cleaningRooms.map((room, roomIndex) => {
                  const Icon = room.icon;

                  const state = roomStateClasses(roomIndex, activeStage);
                  ...
                  return (
                    <motion.button
                      key={room.name}
                      type="button"
                      onClick={() => setActiveStage(roomIndex)}
                      layout
                      whileHover={{
                        y: -3,
                        scale: 1.01,
                    ...
                    >
                      <motion.div
                        ...
                        animate={
                          isActive && !prefersReducedMotion
                            ...
                            : {
                                opacity: isComplete ? 0.45 : 0.2,
                              }
                        }
                        transition={{
                          duration: 2.4,
                          repeat: isActive && !prefersReducedMotion ? 0 : 0,
                        }}
                      />
                        ...
                        <div aria-hidden="true" className="absolute inset-0">
                          {Array.from({
                            ...
                            <motion.span
                              ...
                              style={{
                                left: `${20 + particleIndex * 14}%`,
                                top: `${68 - (particleIndex % 2) * 30}%`,
                              }}
                              animate={
                                prefersReducedMotion
                                  ? undefined
                              ...
                              transition={{
                                duration: 2.2 + particleIndex * 0.25,
                      ...
                      <div className="relative flex h-full flex-col">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          >
                            {isComplete ? "Clean" : isActive ? "In progress" : "Waiting"}
                          </span>
                        </div>
              ...
              <motion.div
                ...
                animate={{
                  left: currentMarkerPosition.left,
                  top: currentMarkerPosition.top,
                }}
                transition={{
                  type: "spring",
                  stiffness: 90,
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <AnimatePresence mode="wait">
                      ...
                      >
                        <p aria-live="polite" className="mt-1 text-sm font-bold text-navy">
            ...
            <div className="mt-3 rounded-[1.5rem] border border-primary/10 bg-white px-4 py-4">
              <div className="flex items-center justify-between gap-4">
                <p className="text-xs font-bold text-navy">Cleaning journey</p>
              ...
              <div className="mt-4 grid grid-cols-4 gap-2">
                {cleaningStages.map((stage, index) => (
                  ...
                  >
                    ...
                    >
                      {index < activeStage ? <Check className="h-3.5 w-3.5" /> : index + 1}
                    </span>

                    <span
                      className={`truncate text-[9px] font-bold sm:text-[10px] ${
                        index === activeStage ? "text-primary" : "text-slate-400"
                      }`}
                    >
                      {stage.shortTitle}
                    </span>
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-xs font-bold text-navy">4 spaces · 1 clean route</p>
              </div>
            </div>
          </motion.div>
          ...
          >
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <p className="text-xs font-bold text-emerald-800">Eco option</p>

                  <p className="text-[9px] text-emerald-600/70">Available at booking</p>
                </div>
              </div>
            </motion.div>
          </motion.div>
      ...
      <div className="absolute bottom-0 left-0 right-0 hidden h-12 items-center border-t border-primary/10 bg-white/35 px-8 backdrop-blur-lg lg:flex">
        <div className="mx-auto flex w-full max-w-[1450px] items-center justify-between">
          <div className="flex items-center gap-5">
            ...
            <span className="text-[10px] font-semibold text-slate-500">Booking system ready</span>
          </div>
