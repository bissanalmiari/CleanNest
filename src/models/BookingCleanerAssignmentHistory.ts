...
export type CleanerAssignmentAction =
  (typeof ASSIGNMENT_ACTIONS)[number];

export interface IBookingCleanerAssignmentHistory
  extends Document {
...
const bookingCleanerAssignmentHistorySchema =
  new Schema<IBookingCleanerAssignmentHistory>(
    {
      bookingId: {
      ...
      previousCleanerName: {
        ...
        maxlength: [
          120,
          "Previous cleaner name cannot exceed 120 characters.",
        ],
        default: undefined,
      },
      ...
      newCleanerName: {
        ...
        maxlength: [
          120,
          "New cleaner name cannot exceed 120 characters.",
        ],
        default: undefined,
      },
      ...
      action: {
        ...
        enum: {
          ...
          message:
            "Cleaner-assignment action is invalid.",
        },
        required: [
          true,
          "Cleaner-assignment action is required.",
        ],
        index: true,
      },
      ...
      changedByUserId: {
        ...
        required: [
          true,
          "The admin who changed the cleaner assignment is required.",
        ],
      },
...
bookingCleanerAssignmentHistorySchema.pre(
  "validate",
  function validateAssignmentHistory() {
    const previousName =
      this.previousCleanerName?.trim();

    const newName =
      this.newCleanerName?.trim();

    if (
      this.action === "assigned" &&
    ...
    if (
      ...
      this.invalidate(
        "action",
        "A reassigned action requires two different cleaner names.",
      );
    }
...
const BookingCleanerAssignmentHistoryModel =
  (mongoose.models
    .BookingCleanerAssignmentHistory as
    | Model<IBookingCleanerAssignmentHistory>
    | undefined) ??
