...
const bookingStatusHistorySchema = new Schema(
  {
    ...
    previousStatus: {
      ...
      enum: {
        ...
        message: "Previous booking status is invalid.",
      },
      default: undefined,
    },
    ...
    newStatus: {
      ...
      enum: {
        values: BOOKING_STATUSES,
        message: "New booking status is invalid.",
      },
      required: [true, "New booking status is required."],
      index: true,
    },
 ...
 */
bookingStatusHistorySchema.pre("validate", function validateStatusChange() {
  if (this.previousStatus !== undefined && this.previousStatus === this.newStatus) {
    this.invalidate(
      "newStatus",
...
export type BookingStatusHistory = InferSchemaType<typeof bookingStatusHistorySchema>;

export type BookingStatusHistoryDocument = HydratedDocument<BookingStatusHistory>;

const BookingStatusHistoryModel =
  (models.BookingStatusHistory as Model<BookingStatusHistory> | undefined) ??
  model<BookingStatusHistory>("BookingStatusHistory", bookingStatusHistorySchema);
