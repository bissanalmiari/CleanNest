...
const RESCHEDULE_SOURCES = [
  "customer",
  "admin",
] as const;

export type RescheduleSource =
  (typeof RESCHEDULE_SOURCES)[number];

const TIME_PATTERN =
  /^([01]\d|2[0-3]):([0-5]\d)$/;

export interface IBookingRescheduleHistory
  extends Document {
...
const bookingRescheduleHistorySchema =
  new Schema<IBookingRescheduleHistory>(
    {
      bookingId: {
      ...
      previousBookingDate: {
        ...
        required: [
          true,
          "Previous booking date is required.",
        ],
      },
      ...
      previousStartTime: {
        ...
        required: [
          true,
          "Previous start time is required.",
        ],
        trim: true,
        match: [
          TIME_PATTERN,
          "Previous start time must use the HH:mm format.",
        ],
      },
      ...
      previousEndTime: {
        ...
        required: [
          true,
          "Previous end time is required.",
        ],
        trim: true,
        match: [
          TIME_PATTERN,
          "Previous end time must use the HH:mm format.",
        ],
      },
      ...
      newStartTime: {
        ...
        match: [
          TIME_PATTERN,
          "New start time must use the HH:mm format.",
        ],
      },
      ...
      newEndTime: {
        ...
        match: [
          TIME_PATTERN,
          "New end time must use the HH:mm format.",
        ],
      },
      ...
      previousDurationMinutes: {
        ...
        min: [
          1,
          "Previous duration must be at least one minute.",
        ],
        max: [
          1440,
          "Previous duration cannot exceed 24 hours.",
        ],
      },
      ...
      newDurationMinutes: {
        ...
        min: [
          1,
          "New duration must be at least one minute.",
        ],
        max: [
          1440,
          "New duration cannot exceed 24 hours.",
        ],
      },
      ...
      changedByUserId: {
        ...
        required: [
          true,
          "The user who rescheduled the booking is required.",
        ],
      },
      ...
      source: {
        ...
        enum: {
          ...
          message:
            "Reschedule source must be customer or admin.",
        },
        required: true,
      },
    },
 ...
 */
bookingRescheduleHistorySchema.pre(
  "validate",
  function validateTimeRanges() {
    ...
    if (
      ...
      if (newEnd <= newStart) {
        this.invalidate(
          "newEndTime",
          "New end time must be later than the new start time.",
        );
      }
    }
  },
...
bookingRescheduleHistorySchema.pre(
  "validate",
  function validateScheduleChanged() {
    if (
    ...
    const sameDate =
      this.previousBookingDate.getTime() ===
      this.newBookingDate.getTime();
...
const BookingRescheduleHistoryModel =
  (mongoose.models
    .BookingRescheduleHistory as
    | Model<IBookingRescheduleHistory>
    | undefined) ??
