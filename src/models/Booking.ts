...
const BOOKING_SOURCES = ["customer", "admin"] as const;

const BOOKING_STATUSES = ["pending", "confirmed", "in_progress", "completed", "cancelled"] as const;

const BOOKING_FREQUENCIES = ["one_time", "weekly", "biweekly", "monthly"] as const;

const PROPERTY_TYPES = ["apartment", "house", "office", "other"] as const;

const PAYMENT_METHODS = ["cash", "card"] as const;

const PAYMENT_STATUSES = ["unpaid", "pending", "paid", "refunded", "failed"] as const;
...
const bookingSchema = new Schema<IBooking>(
  {
    bookingNumber: {
      ...
      trim: true,
      uppercase: true,
      maxlength: [40, "Booking number cannot exceed 40 characters."],
    },
    ...
    createdByUserId: {
      type: Schema.Types.ObjectId,
      ref: "User",
      required: [true, "The user who created the booking is required."],
    },
    ...
    frequency: {
      type: String,
      enum: {
        values: BOOKING_FREQUENCIES,
        message: "Invalid booking frequency.",
      },
      default: "one_time",
      required: true,
    },
    ...
    startTime: {
      ...
      trim: true,
      match: [TIME_PATTERN, "Start time must use the HH:mm format."],
    },
    ...
    endTime: {
      ...
      trim: true,
      match: [TIME_PATTERN, "End time must use the HH:mm format."],
    },

    estimatedDurationMinutes: {
      type: Number,
      min: [1, "Estimated duration must be at least one minute."],
    ...
    propertySize: {
      type: Number,
      min: [1, "Property size must be greater than zero."],
    ...
    addOnsAmount: {
      type: Number,
      required: true,
      min: [0, "Add-ons amount cannot be negative."],
      default: 0,
    },
    ...
    serviceAreaFee: {
      type: Number,
      required: true,
      min: [0, "Service-area fee cannot be negative."],
      default: 0,
    },
    ...
    assignedCleanerName: {
      type: String,
      trim: true,
      minlength: [2, "Cleaner name must contain at least two characters."],
      maxlength: [120, "Cleaner name cannot exceed 120 characters."],
      default: undefined,
    },
...
bookingSchema.pre("validate", function validateBookingTimes() {
  ...
  this.estimatedDurationMinutes = endMinutes - startMinutes;
});
 ...
 * - discountAmount
 */
bookingSchema.pre("validate", function validateBookingPricing() {
  const baseAmount = Number(this.baseAmount) || 0;

  const addOnsAmount = Number(this.addOnsAmount) || 0;

  const serviceAreaFee = Number(this.serviceAreaFee) || 0;

  const discountAmount = Number(this.discountAmount) || 0;

  const subtotal = baseAmount + addOnsAmount + serviceAreaFee;

  if (discountAmount > subtotal) {
    this.invalidate("discountAmount", "Discount cannot exceed the booking subtotal.");

    return;
  }

  this.baseAmount = roundMoney(baseAmount);

  this.serviceBaseAmount = roundMoney(Number(this.serviceBaseAmount) || baseAmount);

  this.propertyAdjustmentAmount = roundMoney(Number(this.propertyAdjustmentAmount) || 0);

  this.addOnsAmount = roundMoney(addOnsAmount);

  this.serviceAreaFee = roundMoney(serviceAreaFee);

  this.discountAmount = roundMoney(discountAmount);

  this.totalAmount = roundMoney(Math.max(0, subtotal - discountAmount));
});
 ...
 */
bookingSchema.pre("validate", function validateCancellationData() {
  ...
  if (this.isModified("status")) {
    ...
    this.cancelledByUserId = undefined;
  }
});
