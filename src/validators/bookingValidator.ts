...
import type { BookingFrequency, BookingStatus, PaymentMethod, PropertyType } from "@/types/enums";

/*
 * CleanNest operates in Lebanon.
 * Booking dates and times are interpreted using Beirut time.
 */
export const CLEAN_NEST_TIME_ZONE = "Asia/Beirut";

export const BOOKING_CHANGE_LIMIT_HOURS = 24;
...
const objectIdSchema = z
  .string()
  .trim()
  .regex(MONGODB_ID_PATTERN, "A valid MongoDB ID is required.");
...
const timeSchema = z.string().trim().regex(TIME_PATTERN, "Time must use the HH:mm format.");

const bookingDateSchema = z
  .string()
  .trim()
  .regex(DATE_PATTERN, "Date must use the YYYY-MM-DD format.")
  .refine((value) => isValidCalendarDate(value), "The selected date is invalid.");

const optionalText = (maximumLength: number, message: string) =>
  z.string().trim().max(maximumLength, message).optional().or(z.literal(""));
...
function formatDateInTimeZone(date: Date, timeZone: string) {
  ...
  if (!year || !month || !day) {
    throw new Error("Unable to format the booking date.");
  }
...
function getTimeZoneOffsetMilliseconds(date: Date, timeZone: string) {
  ...
  const representedUtcTime = Date.UTC(year, month - 1, day, hour, minute, second);
...
export function bookingDateTimeToUtc(
  ...
  if (!isValidCalendarDate(bookingDate) || !TIME_PATTERN.test(time)) {
  ...
  const firstOffset = getTimeZoneOffsetMilliseconds(initialUtcGuess, timeZone);

  const adjustedDate = new Date(initialUtcGuess.getTime() - firstOffset);

  const secondOffset = getTimeZoneOffsetMilliseconds(adjustedDate, timeZone);
...
function validateSchedule(
  ...
  try {
    const scheduledStart = bookingDateTimeToUtc(values.bookingDate, values.startTime);

    if (scheduledStart.getTime() <= Date.now()) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["bookingDate"],
        message: "The booking must be scheduled for a future date and time.",
      });
    }
  } catch {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["bookingDate"],
      message: "The selected booking date and time are invalid.",
    });
  }
}

export const bookingPropertySchema = z
  .object({
    ...
    propertySize: z.coerce
      ...
      .max(100000, "Property size is too large.")
      .optional(),
  })
  .superRefine((property, context) => {
    ...
    if (property.propertyType === "office" && property.propertySize === undefined) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["propertySize"],
        message: "Office size is required.",
      });
    }
  });
...
export const bookingAddOnSelectionSchema = z.object({
  ...
  quantity: z.coerce
    ...
    .max(20, "Add-on quantity cannot exceed 20."),
});
...
const bookingSelectionFields = {
  ...
  promoCodeId: optionalObjectIdSchema,

  frequency: z.enum(BOOKING_FREQUENCIES).default("one_time"),

  property: bookingPropertySchema,

  addOns: z
    .array(bookingAddOnSelectionSchema)
    .max(20, "A booking cannot contain more than 20 add-ons.")
    .default([]),

  paymentMethod: z.enum(PAYMENT_METHODS),

  customerNotes: optionalText(1000, "Customer notes cannot exceed 1000 characters."),
};
...
export const createBookingSchema = createBookingBaseSchema.superRefine((values, context) => {
  validateSchedule(values, context);

  const uniqueAddOnIds = new Set(values.addOns.map((addOn) => addOn.addOnId));

  if (uniqueAddOnIds.size !== values.addOns.length) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["addOns"],
      message: "The same add-on cannot be selected more than once.",
    });
  }
});

export const adminCreateBookingSchema = createBookingBaseSchema
  .extend({
    customerId: objectIdSchema,

    assignedCleanerName: optionalText(120, "Cleaner name cannot exceed 120 characters."),

    adminNotes: optionalText(2000, "Admin notes cannot exceed 2000 characters."),
  })
...
export const editBookingSchema = z
  .object({
    ...
    serviceAreaId: objectIdSchema.optional(),

    promoCodeId: optionalObjectIdSchema,

    property: bookingPropertySchema.optional(),
  ...
  .superRefine((values, context) => {
    ...
    const hasAtLeastOneValue = Object.values(values).some((value) => value !== undefined);
...
export const rescheduleBookingSchema = z
  .object({
    ...bookingScheduleFields,
    reason: optionalText(500, "Rescheduling reason cannot exceed 500 characters."),
  })
  .superRefine((values, context) => {
    validateSchedule(values, context);
  });
...
/*
 * Used by the Cleaning Plan catalogue to price every
 * displayed service in one request.
 */
export const bookingBatchPricePreviewSchema = z.object({
  serviceIds: z
    .array(objectIdSchema)
    .min(1, "At least one service is required.")
    .max(50, "A maximum of 50 services can be priced at once.")
    .refine(
...
export const assignCleanerNameSchema = z.object({
  /*
   * Empty string or null removes the current
   * cleaner-name assignment.
   */
  assignedCleanerName: z.union([
    z
      .string()
      .trim()
      .max(120, "Cleaner name cannot exceed 120 characters.")
      .refine(
  ...
  note: z
    .string()
    .trim()
    .max(500, "Assignment note cannot exceed 500 characters.")
    .optional()
    .or(z.literal("")),
});
...
export function checkBookingChangeWindow({
  ...
  const dateText =
    bookingDate instanceof Date
      ? formatDateInTimeZone(bookingDate, CLEAN_NEST_TIME_ZONE)
      : bookingDate.slice(0, 10);

  const scheduledAt = bookingDateTimeToUtc(dateText, startTime);

  const millisecondsUntilBooking = scheduledAt.getTime() - now.getTime();
  ...
  if (status === "completed") {
    return {
      ...
      millisecondsUntilBooking,
      hoursUntilBooking,
      reason: "A completed booking cannot be changed.",
    };
  }
  ...
  if (status === "in_progress") {
    return {
      ...
      millisecondsUntilBooking,
      hoursUntilBooking,
      reason: "A booking in progress cannot be changed.",
    };
  }

  if (millisecondsUntilBooking <= 0) {
    return {
      allowed: false,
      scheduledAt,
      millisecondsUntilBooking,
  ...
  if (hoursUntilBooking < BOOKING_CHANGE_LIMIT_HOURS) {
    return {
      allowed: false,
      scheduledAt,
      millisecondsUntilBooking,
...
export type BookingBatchPricePreviewInput = z.infer<typeof bookingBatchPricePreviewSchema>;

export type AssignCleanerNameInput = z.infer<typeof assignCleanerNameSchema>;
