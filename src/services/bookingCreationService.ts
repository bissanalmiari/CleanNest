...
import { calculateBookingPrice, type BookingPriceQuote } from "@/services/bookingPriceService";
...
async function generateBookingNumber(session: ClientSession) {
  const datePart = bookingNumberDatePart();

  for (let attempt = 0; attempt < MAX_BOOKING_NUMBER_ATTEMPTS; attempt += 1) {
    const randomPart = String(randomInt(100000, 1000000));

    const bookingNumber = `CN-${datePart}-${randomPart}`;

    const alreadyExists = await BookingModel.exists({
      bookingNumber,
    }).session(session);
  ...
  throw new AppError("Unable to generate a booking number. Please try again.", 500);
}
...
async function consumePromoCodeUsage({
  ...
  const promoObjectId = validateObjectId(promoCodeId, "Promo-code ID");

  const promoCode = await PromoCodeModel.findById(promoObjectId)
...
export async function createCustomerBooking({
  ...
  const creatorObjectId = validateObjectId(createdByUserId, "Creator ID");

  const serviceObjectId = validateObjectId(input.serviceId, "Service ID");

  const addressObjectId = validateObjectId(input.addressId, "Address ID");

  const serviceAreaObjectId = validateObjectId(input.serviceAreaId, "Service-area ID");
  ...
  const serviceArea = await ServiceAreaModel.findById(serviceAreaObjectId).exec();
  ...
  if (address.serviceAreaId) {
    if (address.serviceAreaId.toString() !== serviceAreaObjectId.toString()) {
    ...
    const serviceAreaCity = normalizeLocationText(serviceArea.city);

    const serviceAreaName = normalizeLocationText(serviceArea.area);

    if (addressCity !== serviceAreaCity || addressArea !== serviceAreaName) {
  ...
  const calculatedEndTime = calculateEndTime(input.startTime, estimatedDurationMinutes);
   ...
   */
  const bookingDate = bookingDateTimeToUtc(input.bookingDate, "00:00");
  ...
  try {
    ...
    const createdBooking = await session.withTransaction(async () => {
      ...
      const bookingDocuments = await BookingModel.create(
        [
          {
            bookingNumber,
            ...
            createdByUserId: creatorObjectId,

            serviceAreaId: serviceAreaObjectId,

            promoCodeId: priceQuote.promoCode
              ? new Types.ObjectId(priceQuote.promoCode.id)
              : undefined,
            ...
            propertySize: input.property.propertySize,
            ...
            paymentStatus: input.paymentMethod === "card" ? "pending" : "unpaid",

            assignedCleanerName: assignedCleanerName || undefined,
      ...
      const booking = bookingDocuments.at(0);

      if (!booking) {
        throw new AppError("The booking could not be created.", 500);
      }

      const bookingId = booking._id as Types.ObjectId;
        ...
        [
          {
            ...
            provider: booking.paymentMethod === "cash" ? "cash" : "test_card",
        ...
        [
          {
            ...
            changedByUserId: creatorObjectId,

            reason:
              source === "admin"
                ? "Onsite booking created by admin."
      ...
      await consumePromoCodeUsage({
        promoCodeId: priceQuote.promoCode?.id,
        customerId: customerObjectId,
        bookingId,
        discountAmount: priceQuote.discountAmount,
        session,
    ...
    if (!createdBooking) {
      throw new AppError("The booking transaction did not return a booking.", 500);
    }

    await createNotification({
      userId: customerId,
      type: "booking_created",
      title: source === "admin" ? "Booking created for you" : "Booking request received",
      message:
        source === "admin"
          ? `Our team created onsite booking ${createdBooking.bookingNumber} for you.`
          : `Booking ${createdBooking.bookingNumber} was created and is waiting for approval.`,
      href: "/bookings",
    ...
    return {
      booking: {
        ...
        frequency: createdBooking.frequency,
        ...
        estimatedDurationMinutes:
          createdBooking.estimatedDurationMinutes ?? estimatedDurationMinutes,

        property: {
          type: createdBooking.propertyType,

          bedrooms: createdBooking.bedrooms,

          bathrooms: createdBooking.bathrooms,

          size: createdBooking.propertySize,
        },

        assignedCleanerName: createdBooking.assignedCleanerName ?? null,

        paymentMethod: createdBooking.paymentMethod,

        paymentStatus: createdBooking.paymentStatus,

        customerNotes: createdBooking.customerNotes ?? null,
...
export async function createAdminBooking({
  ...
  const { customerId, assignedCleanerName, adminNotes, ...bookingInput } = input;
