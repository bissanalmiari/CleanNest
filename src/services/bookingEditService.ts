...
function fieldWasProvided(
  input: EditBookingInput,
  fieldName: keyof EditBookingInput,
) {
...
export async function editCustomerBooking({
  ...
  const currentBookingAddOns =
    await BookingAddonModel.find({
  ...
  const nextAddressIdText =
    input.addressId ??
    booking.addressId.toString();

  const nextServiceAreaIdText =
    input.serviceAreaId ??
    booking.serviceAreaId.toString();

  const nextAddressObjectId =
    validateObjectId(
      nextAddressIdText,
      "Address ID",
    );

  const nextServiceAreaObjectId =
    validateObjectId(
      nextServiceAreaIdText,
      "Service-area ID",
    );
  ...
  const {
    ...
    await validateAddressAndServiceArea({
      customerId:
        customerObjectId,
      ...
      serviceAreaId:
        nextServiceAreaObjectId,
    });

  const nextProperty:
    BookingPricePreviewInput["property"] =
      input.property ?? {
        propertyType:
  ...
  const nextAddOns:
    BookingPricePreviewInput["addOns"] =
      input.addOns ??
      currentBookingAddOns.map(
        (addOn) => ({
          addOnId:
  ...
  const promoCodeWasProvided =
    fieldWasProvided(
      input,
      "promoCodeId",
    );

  const nextPromoCodeId =
    promoCodeWasProvided
      ? input.promoCodeId?.trim() ||
        undefined
      ...
      await checkBookingAvailability(
        {
          ...
          serviceAreaId:
            nextServiceAreaObjectId.toString(),
  ...
  try {
    const updatedBooking =
      await session.withTransaction(
        async () => {
          ...
          transactionalBooking.addressId =
            nextAddressObjectId;

          transactionalBooking.serviceAreaId =
            nextServiceAreaObjectId;

          transactionalBooking.promoCodeId =
            appliedPromoCodeId
              ? new Types.ObjectId(
                  appliedPromoCodeId,
                )
          ...
          transactionalBooking.propertyType =
            nextProperty.propertyType;

          transactionalBooking.bedrooms =
            nextProperty.bedrooms;

          transactionalBooking.bathrooms =
            nextProperty.bathrooms;

          transactionalBooking.propertySize =
          ...
          transactionalBooking.serviceAreaFee =
            serviceAreaFee;

          transactionalBooking.discountAmount =
            priceQuote.discountAmount;

          transactionalBooking.totalAmount =
            totalAmount;

          transactionalBooking.paymentMethod =
            nextPaymentMethod;
           ...
           */
          if (
            transactionalBooking.paymentStatus !==
              "paid" &&
            transactionalBooking.paymentStatus !==
              "refunded"
          ) {
          ...
          transactionalBooking.customerNotes =
            nextCustomerNotes;
    ...
    if (!updatedBooking) {
      throw new AppError(
        "The booking update did not return a booking.",
        500,
      );
    }
     ...
     */
    const returnedPromoCodeId =
      updatedBooking.promoCodeId?.toString();
    ...
    if (
      ...
      throw new AppError(
        "The booking promo code could not be updated.",
        500,
      );
    }
    ...
    return {
      booking: {
        ...
        property: {
          type:
            updatedBooking.propertyType,

          bedrooms:
            updatedBooking.bedrooms ??
            null,

          bathrooms:
            updatedBooking.bathrooms ??
            null,

          size:
            updatedBooking.propertySize ??
            null,
        },
